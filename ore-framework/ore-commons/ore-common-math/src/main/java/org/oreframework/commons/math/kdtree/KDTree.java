...
public class KDTree<T> implements Serializable {
    ...
    */
	public List<T> nearestEuclidean(double[] key) throws KeySizeException {
		return nearestDistance(key, new EuclideanDistance());
	}
    ...
    */
	...
	private List<T> nearestDistance(double[] key, DistanceMetric metric) throws KeySizeException {
		...
		for (int i = 0; i < n; ++i) {
			...
			HPoint p = kd.k;
			//kd.
			if (metric.distance(kd.k.coord, key) < 0) {
				nbrs.push(kd.v);
			}
		}
		return nbrs;
